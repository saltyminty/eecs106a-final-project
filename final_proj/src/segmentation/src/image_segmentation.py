...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    copy = np.copy(gray_img)
    copy = np.logical_and(copy > lower_thresh, copy < upper_thresh)
    return copy
...
def edge_detect_naive(gray_img):
    ...
    Ky = np.array([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]).T
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    img_r = np.reshape(img_d, (-1, img_d.shape[-1]))
...
def segment_image(img): 
    ...
    binary = threshold_segment_naive(to_grayscale(img), 0.20*255, 0.30* 255).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    test_thresh_naive(test_img, 0.3*255, 0.8 * 255)
